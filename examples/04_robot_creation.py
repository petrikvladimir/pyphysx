...
class Robot:
    ...
    def find_links(root):
        ...
        return [Link(child.attrib['name'], child.find('collision').find('geometry').find('mesh').attrib['filename']
        if (child.find('visual') is not None) else None)
                for child in root if (child.tag == 'link')]

    @staticmethod
    def find_joints(root):
        return [Joint(child.attrib['name'], child.find('parent').attrib['link']
...
urdf_path = "/home/kzorina/PandaRobot.jl/deps/Panda/panda.urdf"
    ...
    for joint in joints_list_copy_copy:
        if joint.parent in position_dict:
            ...
            print("Parent {} pose = {} {}, joint pose = {} {}, child {} pose = {} {}"\
                  .format(joint.parent,position_dict[joint.parent][0], R.from_quat(position_dict[joint.parent][1]).as_euler("xyz", degrees=True),
...
for link in robot.links:
    if link.mesh_path is not None:
        ...
        obj = trimesh.load(link.mesh_path.replace("package://", "/home/kzorina/PandaRobot.jl/deps/"), split_object=True, group_material=False)
